...
data = pd.read_csv("Atrribute_Dataset.csv")
df = pd.DataFrame(data)

for row in df.itertuples():

    myCursor.execute('''
                INSERT INTO iNeuronTasksClass (Dress_ID , Style ,	Price , Rating , Size , Season , NeckLine , SleeveLength , waiseline , Material ,FabricType	, Decoration )
                VALUES (?,?,?)
                ''',
                    row.Dress_ID ,
                     row.Style ,
